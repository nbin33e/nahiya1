...
const SetupScreen: React.FC<SetupScreenProps> = ({ numPlayers, onComplete, onBack }) => {
  ...
  return (
    <div className="h-full flex-1 flex flex-col items-center bg-white overflow-hidden font-['Changa']">
      ...
      <div className="flex-1 w-full max-w-4xl p-6 flex flex-col justify-center gap-8 overflow-y-auto min-h-0">
        <div className="w-full text-center shrink-0">
          ...
          <input 
            ...
            className="w-full max-w-md mx-auto px-6 py-4 rounded-[25px] border-4 border-blue-200 text-3xl font-black focus:border-yellow-400 outline-none text-center shadow-md bg-white text-blue-900"
        ...
        <div className="w-full">
          <p className="text-2xl font-black text-blue-900 mb-6 text-center">اختر الشخصية:</p>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {CHARACTERS.map((char) => (
              <button
                key={char.id}
                onClick={() => updatePlayer('character', char)}
                className={`p-4 rounded-[40px] border-4 transition-all flex flex-col items-center gap-3 ${
                  currentPlayer?.character?.id === char.id 
                    ? 'border-yellow-400 bg-yellow-50 scale-105 shadow-xl' 
                    : 'border-blue-50 bg-white'
                }`}
              >
                <div className={`p-2 rounded-full ${char.color} border-4 border-white shadow-md`}>
                  <img src={char.image} alt={char.name} className="w-24 h-24 md:w-32 md:h-32 rounded-full" />
                </div>
                <span className="font-black text-lg">{char.name}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="w-full p-6 bg-blue-50 border-t-4 border-blue-100 flex justify-center shrink-0">
        <button 
          disabled={!isReady}
          onClick={handleNext}
          className={`w-full max-w-sm py-5 rounded-[30px] text-3xl font-black transition-all ${
            isReady 
              ? 'bg-yellow-400 text-blue-900 shadow-[0_8px_0_#1e3a8a]' 
